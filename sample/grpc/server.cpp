    ...
    public:
        ...
        asyncio::task::Task<void> write(const T element) {
            ...
            if (!co_await asyncio::task::Cancellable{
                ...
                throw co_await asyncio::error::StacktraceError<
                    std::system_error>::make(asyncio::task::Error::Cancelled);
        }
    ...
    protected:
                ...
                auto task = asyncio::task::spawn(
                    [
                        ...
                        const auto response = co_await
                            asyncio::error::capture(std::invoke(handler, std::move(request)));
        ...
        handle(
            ...
            while (true) {
                ...
                auto task = asyncio::task::spawn(
                    [&handler, context = std::move(context), reader = std::move(reader)
                    ]() -> asyncio::task::Task<void> {
        ...
        handle(
            ...
            while (true) {
                ...
                auto task = asyncio::task::spawn(
                    [&handler, context = std::move(context), stream = std::move(stream)
                    ]() -> asyncio::task::Task<void> {
                        const auto result = co_await asyncio::error::capture(
                            std::invoke(handler, Stream{context, stream}));
