...
ASYNC_TEST_CASE("post task to thread pool with exception catching", "[thread]") {
    using namespace std::chrono_literals;
    ...
    SECTION("normal") {
        SECTION("void") {
            REQUIRE_NOTHROW(co_await asyncio::toThreadPoolCatching([] {
                std::this_thread::sleep_for(50ms);
            }));
            REQUIRE(std::chrono::system_clock::now() - tp > 45ms);
        }
        ...
        SECTION("not void") {
            const auto result = co_await asyncio::toThreadPoolCatching([] {
                std::this_thread::sleep_for(50ms);
                return 1024;
            });
            REQUIRE(result == 1024);
    ...
    SECTION("cancel") {
        ...
        SECTION("void") {
            auto task = asyncio::toThreadPoolCatching([] {
                std::this_thread::sleep_for(50ms);
            });
            REQUIRE(task.cancel());
            REQUIRE_THROWS_MATCHES(
...
ASYNC_TEST_CASE("post cancellable task to thread pool with exception catching", "[thread]") {
    const auto tp = std::chrono::system_clock::now();
