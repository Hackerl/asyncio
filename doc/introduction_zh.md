...
`asyncio` 在以下几个方面可能优于现有的协程库：

- 错误码与异常相结合，完善的错误处理机制。
- 简答直接的任务取消方式，类似于 `Python` 的 `asyncio` —— `task.cancel()`。
- 多种子任务聚合方式，结构化并发模型，借鉴于 `JavaScript` 的 `Promise`。
- 灵活的动态任务管理方案，类似于 `Golang` 的 `WaitGroup`。
- 内置调用栈跟踪功能，可自顶向下遍历任务树，也可自底向上完成 `backtrace`。
