...
namespace asyncio::net::tls {
    ...
    private:
        task::Task<std::size_t, std::error_code> transferIn() {
            ...
            const int result = BIO_write(SSL_get_rbio(mSSL.get()), data.data(), static_cast<int>(*n));
            assert(result == *n);
            co_return {};
        }
    ...
    public:
        task::Task<std::size_t, std::error_code> handshake() {
            while (true) {
                const int result = SSL_do_handshake(mSSL.get());
                ...
                if (const int error = SSL_get_error(mSSL.get(), result); error == SSL_ERROR_WANT_READ) {
                    CO_EXPECT(co_await transferOut());
        ...
        task::Task<std::size_t, std::error_code> read(const std::span<std::byte> data) override {
            while (true) {
                const int result = SSL_read(mSSL.get(), data.data(), static_cast<int>(data.size()));
                ...
                if (result == 0) {
                    if (const int res = SSL_shutdown(mSSL.get()); res != 1) {
                        assert(res < 0);
        ...
        task::Task<std::size_t, std::error_code> write(const std::span<const std::byte> data) override {
            while (true) {
                const int result = SSL_write(mSSL.get(), data.data(), static_cast<int>(data.size()));
        ...
        task::Task<void, std::error_code> close() override {
            while (true) {
                const int result = SSL_shutdown(mSSL.get());
